 const cssInput = document.getElementById("css-input");
        ...
        function checkAnswer() {
            ...
            try {
                // ขยับกบตามโค้ดที่พิมพ์ โดยคงค่าแนวนอน (justify-content) ไว้เหมือนเดิม
                frogLayer.style.cssText = `justify-content: center; gap: 40px; align-items: flex-start; ${inputCode}`;
            } catch (e) { }

            // ตรวจหาคำตอบที่ถูกต้อง (ใช้ align-items: flex-end)
            const cleanInput = inputCode.replace(/\s/g, '').toLowerCase();
            const isCorrect = cleanInput.includes('align-items:flex-end');
        ...
        function showAnswer() {
            // คำตอบที่ถูกต้องคือ align-items: flex-end;
            const answer = "align-items: flex-end;";
            let i = 0;
